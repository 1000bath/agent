...
Example extensions for Dek Agent.

## Usage

```bash
# From the repository root, load an extension with --extension
./dek-agent.sh --extension packages/coding-agent/examples/extensions/permission-gate.ts
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs the Dek Agent theme with macOS dark/light mode |
