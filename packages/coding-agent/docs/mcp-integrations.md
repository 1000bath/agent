...
Connect external services (Linear, Notion, …) to Dek Agent over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with Dek Agent's single-tool design, MCP integrations are **not**
...
`pyproject.toml` (depends on `mcp`, `httpx`, and `dek-agent-runtime`):

```toml
[project]
name = "dek-agent-skill-acme"
version = "0.1.0"
requires-python = ">=3.10"
dependencies = ["mcp", "httpx", "dek-agent-runtime"]
