...
<h3 align="center">
Copider Code: A Self-Improving RLM Agent
</h3>
...
Copider Code is an open-source coding and research agent for general and long-running work. It is designed around two core abstractions:
...
- The **[Continual Harness](https://arxiv.org/abs/2605.09998)** stores supplemental prompts, memories, skill descriptions, and reusable subagent specifications as durable state that Copider Code can refine through small, evidence-backed updates, local to the session by default.

Copider Code combines a persistent Python control environment with durable harness state, so useful working context and reusable operating patterns can outlive a single chat window.
...
Install the latest stable release on macOS or Linux (upstream distribution):
...
Start Copider Code from the repository or directory you want it to work in:
...
On first launch, run `/login` to choose a subscription or API-key provider. Copider Code works in the current directory and can run commands and modify files there. Use a disposable clone, clean worktree, or another checkpoint you can inspect and restore.

> [!WARNING]
> Copider Code executes model-generated Python and project commands with your user permissions. Its worker and kernel processes improve lifecycle isolation and recovery; they are **not** a security sandbox. Review changes and use trusted repositories, instructions, skills, and extensions only. Run untrusted code or instructions in an external sandbox or restricted environment.

Useful commands:

```bash
dek-agent agents                   # Browse running, idle, and saved sessions
dek-agent attach <agent>           # Reattach to a running session
...
dek-agent update [--force]         # Update Copider Code
dek-agent shutdown [--force]       # Stop every agent, worker, and background service
```

## Dek Integration

This repository is maintained as the Copider Code runtime, based on the upstream Prime Agent implementation. The surrounding Dek packages are kept separate:
...
Copider Code is built for long-running work, especially for evaluations in research. These features are available in the TUI, and when run autonomously.
...
- **Heartbeats and schedules:** `/heartbeat`, `rlm_heartbeat`, and `dek-agent schedule` can re-enter a session periodically or at a specific time.
...
Copider Code is fully open source and released under the [MIT License](LICENSE).
