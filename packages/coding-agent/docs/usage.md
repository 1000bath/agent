...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Dek Agent to receive the shortcut.
...
dek-agent -r [path|id]        # Browse sessions or resume one directly
dek-agent --no-session        # Ephemeral mode; do not save
dek-agent --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `dek-agent agents`, `dek-agent list`, or `dek-agent attach <agent>` to find and reattach to running work. `dek-agent stop <agent>` stops one root agent, while `dek-agent shutdown` stops all workers and the local supervisor.
...
dek-agent agents
dek-agent list [--all]
dek-agent attach <agent>
dek-agent stop <agent>
dek-agent rename <agent> <name>
dek-agent send <agent> <message>
dek-agent schedule <list|add|cancel>
dek-agent status
dek-agent doctor [--fix]
dek-agent shutdown [--force]
...
dek-agent update [--force]
dek-agent config
```

See [Dek Agent Packages](packages.md) for package sources and security notes.
...
Use `dek-agent session export <file> [output]` to export a session to HTML.
...
```bash
dek-agent --no-extensions -e ./my-extension.ts
```
...
Dek Agent keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and Dek Agent packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

Dek Agent preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current Dek Agent architecture.
