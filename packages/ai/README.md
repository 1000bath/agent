<p align="center">
  <a href="https://primeintellect.ai">
    <picture>
      ...
      <img alt="spider mark" src="../../assets/brand/spider-black.svg" width="88">
    </picture>
  </a>
</p>

<h1 align="center">Dek Agent AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the Dek Agent package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install dek-agent-ai
```

TypeBox exports are re-exported from `dek-agent-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'dek-agent-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'dek-agent-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'dek-agent-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'dek-agent-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'dek-agent-ai';
...
```typescript
import { getModel, stream } from 'dek-agent-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'dek-agent-ai';
...
```typescript
import { Model, stream } from 'dek-agent-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Dek Agent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'dek-agent-ai';
...
```typescript
import { getModel, complete, Context } from 'dek-agent-ai';
...
```typescript
import { Context, getModel, complete } from 'dek-agent-ai';
...
- OAuth login flows are not supported in browser environments. Use the `dek-agent-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'dek-agent-ai';
...
```bash
npx dek-agent-ai login              # interactive provider selection
npx dek-agent-ai login anthropic    # login to specific provider
npx dek-agent-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `dek-agent-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'dek-agent-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'dek-agent-ai/oauth';
...
```typescript
import { getModel, complete } from 'dek-agent-ai';
import { getOAuthApiKey } from 'dek-agent-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `dek-agent-ai`
