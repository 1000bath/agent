...
Dek Agent also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Dek Agent uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Dek Agent AI)
...
### Extended Message Types (from Dek Agent Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Dek Agent (legacy field name)
