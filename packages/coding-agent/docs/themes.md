> Dek Agent can create themes. Ask it to build one for your setup.
...
On first run, Dek Agent detects your terminal background and defaults to `dark` or `light`.
...
**Hot reload:** When you edit the currently active custom theme file, Dek Agent reloads it automatically for immediate visual feedback.
...
Dek Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Dek Agent falls back to the nearest approximation.
