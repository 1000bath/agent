...
Dek Agent saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.
...
```bash
dek-agent --continue          # Continue the most recent session
dek-agent --resume [path|id]  # Browse past sessions or resume one directly
dek-agent --no-session        # Ephemeral mode; do not save
dek-agent --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `dek-agent --resume` opens the same picker at startup, and `dek-agent --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `dek-agent --resume -- "continue this work"`.
...
When available, Dek Agent uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `dek-agent --resume`.
...
When `/tree` switches away from one branch to another, Dek Agent can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
