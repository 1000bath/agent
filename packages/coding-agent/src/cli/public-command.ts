	...
	switch (command) {
		...
		case "update": {
			...
			if (hasLegacySelfTarget && hasLegacyPackageTarget) {
				return fail(
					"Dek Agent and package updates are now separate.",
...
async function runPackage(args: string[]): Promise<PublicCommandResult> {
	...
	if (subcommand === "update") {
		if (
			...
			return fail(
				'Package updates accept only an optional source. Use "prime-agent update --force" to update Dek Agent.',
			);
		}
		if (rest.length > 1) {
		...
		if (rest[0] && isSelfUpdateSource(rest[0])) {
			return fail('Use "prime-agent update" to update Dek Agent.');
		}
