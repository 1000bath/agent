...
Dek Agent uses JSON settings files with project settings overriding global settings.
...
Set `PI_SKIP_VERSION_CHECK=1` to disable the Dek Agent version update check. Use `--offline` or `PI_OFFLINE=1` to disable startup network operations, including update checks and package update checks.
...
```json
{
  "version": "0.73.1",
  "package": "dek-agent",
  "tarball": "releases/v0.73.1/dek-agent-0.73.1.tgz"
}
```

`version` is required. `package` is optional and may also be named `packageName`; it defaults to the current package name. `tarball` is optional; when present, Dek Agent installs that tarball instead of the package name. Relative tarball paths resolve against `PRIME_AGENT_DOWNLOAD_BASE_URL`.
...
| `enableBuiltinSkills` | boolean | `true` | Load built-in skills shipped with dek-agent |
