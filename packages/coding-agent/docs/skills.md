> Dek Agent can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that Dek Agent loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

Dek Agent implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent IPython kernel.
...
- [Creating Skills with Dek Agent](#creating-skills-with-dek-agent)
...
- Built-in: `skills/` shipped with the dek-agent package (lowest precedence)
...
Dek Agent ships with built-in skills that load by default:
...
```python
print(await websearch("latest Dek Agent release"))
```
...
1. At startup, Dek Agent scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, Dek Agent exposes `web_search` in IPython. If the module defines `run()`, the module is wrapped as an async callable:
...
Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.prime/agent/kernel-venv`; set `PRIME_AGENT_KERNEL_VENV` to override it. If `pyproject.toml` changes, Dek Agent rebuilds the kernel venv so dependency changes are picked up.

If you set `PRIME_AGENT_KERNEL_PYTHON`, Dek Agent does not install packages into that environment. The Python must already have `ipykernel`, `dek-agent-runtime`, and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
## Creating Skills with Dek Agent

Dek Agent ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh Dek Agent session after adding a Python-backed skill so kernel setup can install and import the package.
...
Dek Agent validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
