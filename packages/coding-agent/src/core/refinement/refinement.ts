...
const REFINEMENT_SYSTEM_PROMPT = `You are Dek Agent's /refine continual harness subsystem.
...
skills, and subagent specs that lets Dek Agent improve reusable behavior
...
- The default editable continual harness store is local to the current Dek Agent session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Dek Agent's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Dek Agent session. Global continual harness entries persist across Dek Agent sessions.",
