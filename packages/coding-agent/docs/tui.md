> Dek Agent can create TUI components. Ask it to build one for your use case.
...
Dek Agent shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Dek Agent is streaming a response.
...
// Restore Dek Agent's default spinner
ctx.ui.setWorkingIndicator();
```
