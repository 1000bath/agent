...
## Spider workspace additions
- Preserve package boundaries and legacy APIs, protocols, storage paths, environment variables, aliases, and file formats.
- Use bounded validation and report dependency/platform blockers separately from regressions.
- Never expose secrets; run `git diff --check` before commits and never commit `.codegraph/`.
