...
export class ProviderAuthFlows {
	...
	getLoginProviderOptions(authType?: "oauth" | "api_key"): AuthSelectorProvider[] {
		...
		for (const providerId of modelProviders) {
			if (providerId === PRIME_INFERENCE_PROVIDER_ID) continue;
	...
	async runPrimeAgentTracesLogin(): Promise<AuthenticationResult> {
		...
		try {
			...
			if (result.source === "manual") {
				browserAbort.abort();
				dialog.showProgress("Checking Dek Agent trace access...");
				...
				if (!access.ok) {
					const status = access.status === undefined ? "" : `HTTP ${access.status}: `;
					throw new Error(`Prime API key does not have Dek Agent trace access (${status}${access.message})`);
				}
			}
