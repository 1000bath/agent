...
export class StaleDaemonError extends Error {
	constructor(
		...
		super(
			`An incompatible Dek Agent daemon is running.\n\n${daemonIdentity}\n` +
...
async function ensureDaemonRunning(socketPath: string, spawnCwd?: string): Promise<void> {
	...
	const throwIfFailed = () => {
		...
		if (childFailure.type === "error") {
			throw new Error(`Failed to spawn Dek Agent daemon: ${childFailure.error.message}.${logTail}`);
		}
		const signal = childFailure.signal ? `, signal ${childFailure.signal}` : "";
		throw new Error(
			`Dek Agent daemon exited during startup (code ${childFailure.code ?? "unknown"}${signal}).${logTail}`,
		);
	};
