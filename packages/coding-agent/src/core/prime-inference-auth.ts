...
export const PRIME_AGENT_TRACES_PROVIDER_NAME = "Dek Agent Traces";
...
export async function loginPrimeAgentTraces(
	...
	if (config.apiKey) {
		...
		callbacks.onProgress?.(
			`Existing Prime CLI key cannot upload Dek Agent traces (${formatAccessFailure(access)}). Starting browser login...`,
		);
	} else {
		callbacks.onProgress?.("No Prime CLI API key found. Starting browser login...");
	}
	...
	throwIfCancelled(callbacks.signal);
	callbacks.onProgress?.("Checking Dek Agent trace access...");
	...
	if (!access.ok) {
		throw new Error(`Prime API key does not have Dek Agent trace access (${formatAccessFailure(access)})`);
	}
