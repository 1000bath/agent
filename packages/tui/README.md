...
<h1 align="center">Dek Agent TUI</h1>
...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "dek-agent-tui";
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "dek-agent-tui";
...
```typescript
import { Container, type Focusable, Input } from "dek-agent-tui";
...
Renders Kitty or iTerm2 terminal graphics when supported. Set `fallbackOnly: true` to show compact image metadata instead; Dek Agent uses this metadata-only mode.
...
```typescript
import { CombinedAutocompleteProvider } from "dek-agent-tui";
...
```typescript
import { matchesKey, Key } from "dek-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "dek-agent-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "dek-agent-tui";
import type { Component } from "dek-agent-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "dek-agent-tui";
import type { Component } from "dek-agent-tui";
