...
<h1 align="center">Spider Agent Core</h1>

<p align="center">
  Stateful agent runtime.
</p>

Release docs use the Spider Agent package names. The source workspace manifests still keep inherited package names until the namespace migration is complete.

## Workspace Package

```bash
npm install dek-agent-core
```

## Quick Start

```typescript
import { Agent } from "dek-agent-core";
import { getModel } from "dek-agent-ai";
...
```typescript
import { Agent, streamProxy } from "dek-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "dek-agent-core";
