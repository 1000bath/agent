...
Dek Agent also provides native recursive delegation through `rlm.call()` and
...
- **Isolated context**: Each subagent runs in a separate Dek Agent process
...
This tool executes a separate Dek Agent subprocess with a delegated system prompt and tool/model configuration.
