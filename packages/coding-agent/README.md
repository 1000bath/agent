<p align="center">
  <a href="https://primeintellect.ai">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="../../assets/brand/spider.svg">
      <img alt="spider mark" src="../../assets/brand/spider-black.svg" width="96">
    </picture>
  </a>
</p>

<h1 align="center">Dek Agent CLI</h1>

<p align="center">
  RLM-native terminal coding and research harness.
</p>

Dek Agent began as a hard fork of [pi-mono](https://github.com/badlogic/pi-mono), but it is now developed and distributed independently. This workspace retains inherited `@earendil-works/pi-*` source package identifiers, the `pi` package manifest key, and a source-package `pi` bin entry for internal compatibility. Public releases are currently versioned tarball artifacts installed by the scripts below; release packaging rewrites the application package and command to `dek-agent`. Do not use the inherited npm package as the Dek Agent install path.
...
- [Customization](#customization)
  ...
  - [Themes](#themes)
  - [Dek Agent Packages](#dek-agent-packages)
...
Then just talk to Dek Agent. By default, Dek Agent gives the model one tool: `ipython`. The model uses the persistent kernel to read files, run commands, edit code, and inspect data. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [Dek Agent packages](#dek-agent-packages).
...
For each built-in provider, Dek Agent maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so Dek Agent can receive the follow-up shortcut.
...
```bash
dek-agent -c                  # Continue most recent session
dek-agent -r [path|id]        # Browse past sessions or resume one directly
dek-agent --no-session        # Ephemeral mode (don't save)
dek-agent --fork <path|id>    # Fork specific session file or ID into a new session
```
...
Dek Agent stable builds fetch `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` and remain on the beta channel. Override the base URL with `PRIME_AGENT_DOWNLOAD_BASE_URL`. Disable version checks with `PI_SKIP_VERSION_CHECK=1`.

Use `--offline` or `PI_OFFLINE=1` to disable startup network operations, including update checks and package update checks.

## Context Files

Dek Agent loads `AGENTS.md` (or `CLAUDE.md`) at startup from:
...
Place in `~/.prime/agent/prompts/`, `.prime/agent/prompts/`, or a [Dek Agent package](#dek-agent-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).

### Skills

On-demand capability packages following the [Agent Skills standard](https://agentskills.io). At startup, Dek Agent gives the model each visible skill's name, type, description, and location. The full `SKILL.md` stays out of context until the model inspects it with `ipython` or you explicitly invoke `/skill:name`.
...
Skills can also be Python-backed. A Python skill is a normal skill directory with `SKILL.md` plus a Python package at `src/<import_name>/`. Dek Agent installs it into the persistent IPython kernel and exposes it by import name, so the model can call it directly, inspect it with `help()`, or use any console scripts the skill declares.

Place in `~/.prime/agent/skills/`, `~/.agents/skills/`, `.prime/agent/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [Dek Agent package](#dek-agent-packages) to share with others. See [docs/skills.md](docs/skills.md).

Dek Agent ships with a built-in `websearch` skill (Google search via the [Serper](https://serper.dev) API). It loads by default; run `/login`, switch to **MCP Connections**, and choose "Serper (web search)" to add your key. Disable it with `bundledSkills.websearch: false`, or override it with your own `websearch` skill in any location above. See [docs/skills.md#built-in-skills](docs/skills.md#built-in-skills).
...
TypeScript modules that extend Dek Agent with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. Dek Agent waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make Dek Agent look like Claude Code
- Games while waiting (yes, Doom runs)
- ...anything you can dream up

Place in `~/.prime/agent/extensions/`, `.prime/agent/extensions/`, or a [Dek Agent package](#dek-agent-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and Dek Agent immediately applies changes.

Place in `~/.prime/agent/themes/`, `.prime/agent/themes/`, or a [Dek Agent package](#dek-agent-packages) to share with others. See [docs/themes.md](docs/themes.md).

### Dek Agent Packages

Bundle and share extensions, skills, prompts, and themes via npm or git.

> **Security:** Dek Agent packages run with full system access. Extensions execute arbitrary code, and skills can instruct the model to perform any action including running executables. Review source code before installing third-party packages.

```bash
dek-agent package install npm:@foo/dek-agent-tools
dek-agent package install npm:@foo/dek-agent-tools@1.2.3  # pinned version
dek-agent package install git:github.com/user/repo
dek-agent package install git:github.com/user/repo@v1       # tag or commit
dek-agent package install git:git@github.com:user/repo
dek-agent package install https://github.com/user/repo
dek-agent package install ssh://git@github.com/user/repo
dek-agent package remove npm:@foo/dek-agent-tools
dek-agent package list
dek-agent package update                                  # update packages, except pinned versions
dek-agent package update npm:@foo/dek-agent-tools       # update one package
dek-agent update                                          # update Dek Agent
dek-agent update --force                                  # reinstall Dek Agent even if current
dek-agent config                                          # enable/disable package resources
```
...
```json
{
  "name": "my-dek-agent-package",
  "keywords": ["dek-agent-package"],
  "pi": {
    "extensions": ["./extensions"],
    "skills": ["./skills"],
    "prompts": ["./prompts"],
...
Without a `pi` manifest, Dek Agent auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "dek-agent";
...
Dek Agent is forked from [pi-mono](https://github.com/badlogic/pi-mono) by Mario Zechner and keeps MIT attribution in the root license.

The package architecture, extension model, and source package names still reflect that upstream lineage while the distributed command and release artifacts are branded for Dek Agent.

## CLI Reference

```bash
dek-agent [options] [@files...] [messages...]
```

Run `dek-agent help` for the command list and `dek-agent help <command>` for details.

### Agent Commands

```bash
dek-agent agents                         # Search running, idle, and inactive sessions
dek-agent list [--all]                   # List active or saved agents
dek-agent attach <agent>                 # Attach the interactive UI
dek-agent stop <agent>                   # Stop one agent
dek-agent rename <agent> <name>          # Rename an agent
dek-agent send <agent> <message>         # Send an agent-to-agent message
dek-agent status                         # Show background service status
dek-agent doctor [--fix]                 # Inspect or safely clean up background services
...
```bash
dek-agent schedule list [--all] [agent]
dek-agent schedule add <agent> <schedule> -- <message>
dek-agent schedule cancel <job-id>
```
...
```bash
dek-agent package install <source> [--local]
dek-agent package remove <source> [--local]
dek-agent package list
dek-agent package update [source]
dek-agent update [--force]                   # Update Dek Agent itself
dek-agent config                             # Enable/disable package resources
```
...
In print mode, Dek Agent also reads piped stdin and merges it into the initial prompt:

```bash
cat README.md | dek-agent -p "Summarize this text"
```
...
Use `dek-agent model list [search]` to list available models.
...
Use `dek-agent session export <file> [output]` to export a saved session to HTML.
...
```bash
dek-agent @prompt.md "Answer this"
dek-agent -p @screenshot.png "What's in this image?"
dek-agent @code.ts @test.ts "Review these files"
```
...
dek-agent "List all .ts files in src/"

# Non-interactive
dek-agent -p "Summarize this codebase"

# Non-interactive with piped stdin
cat README.md | dek-agent -p "Summarize this text"

# Different model
dek-agent --provider openai --model gpt-4o "Help me refactor"

# Model with provider prefix (no --provider needed)
dek-agent --model openai/gpt-4o "Help me refactor"

# Model with thinking level shorthand
dek-agent --model sonnet:high "Solve this complex problem"

# Limit model cycling
dek-agent --models "claude-*,gpt-4o"

# Restrict to the built-in IPython tool
dek-agent --tools ipython -p "Review the code"

# High thinking level
dek-agent --thinking high "Solve this complex problem"
```
...
| `PI_SKIP_VERSION_CHECK` | Skip the Dek Agent version update check at startup. This prevents the release manifest request |
| `PRIME_AGENT_DOWNLOAD_BASE_URL` | Override the Dek Agent release manifest and tarball base URL |
...
| `PRIME_AGENT_TRACES_BASE_URL` | Override the Dek Agent trace upload API base URL |
...
- [Dek Agent AI](../ai): Core LLM toolkit
- [Dek Agent Core](../agent): Agent framework
- [Dek Agent TUI](../tui): Terminal UI components
